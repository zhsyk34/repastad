...
function Graph(title,canvasN,type,middleval,local)
{
	...
	this.draw = function()
	{
		...
		//计算左边间距
		if(this.options.type =='line'){
			if(leftSpace < cont.measureText(vMin+maxrange*2).width)
				leftSpace =45;
		}else {
			if(leftSpace < cont.measureText(vMin+vRange).width)
				leftSpace = cont.measureText(vMin+vRange).width+5;
		}
		leftSpace = 50;
        ...
        if(local=='tw'){
			hScale = (canvas.width-leftSpace-rightSpace)/270;
        }else if (local=='cn'){
            ...
            for(var j=vMin;j<=vMax+2;j+=spacing){
                ...
                cont.textBaseline = "middle";
			     ...
			     if(wucha<0.02 || wucha==0){
			     	cont.fillText(middle+'', leftSpace-2, y);	
			     }else {
			     	cont.fillText(verticalVal+'', leftSpace-2, y);
			     }
			     //cont.strokeStyle = "rgb(0,0,0)";
		...
		//画水平底线 
		if(this.options.type=='line'){
                var hdistance = (canvas.width-rightSpace-leftSpace)/4;
                cont.textBaseline="top";
				cont.textAlign = "center";
				cont.fillStyle = jsgraph_textcol;
				var timeArray = null;
				if(local=='cn'){
					...
					//timeArray = ['9:00','10:00','11:00','12:00','13:00'];
					timeArray = ['','','','',''];
				}
				...
				for(var m=1;m<timeArray.length+1;m++){
                    ...
                    //cont.strokeStyle = "rgb(255,0,0)";
				    //cont.beginPath();
				    //cont.moveTo(startP, 200);
				    //cont.lineTo(startP,10);
				    //cont.stroke();
                    //cont.strokeStyle = "rgb(0,0,0)";
                    startP += hdistance;		    
                }
